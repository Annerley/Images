...
void AbstractImage::toColorInversion()
{
	for (size_t i = 0; i < height(); ++i) {
		for (size_t j = 0; j < width(); ++j) {
			uchar b = 256 -_pixelData[i][j].get_blue();
			uchar g = 256 -_pixelData[i][j].get_green();
			uchar r = 256 -_pixelData[i][j].get_red();

			_pixelData[i][j] = { r, g, b, _pixelData[i][j].alpha }; 
...
void AbstractImage::toSepia()
{
	for (size_t i = 0; i < height(); ++i) {
		for (size_t j = 0; j < width(); ++j) {
			...
			uchar b =  _pixelData[i][j].get_blue()*0.114;
			uchar g =  _pixelData[i][j].get_green()*0.587;
			uchar r =  _pixelData[i][j].get_red()*0.299;
...
void AbstractImage::toGrayScale()
{
	{
		for (size_t i = 0; i < height(); ++i) {
			for (size_t j = 0; j < width(); ++j) {

				uchar b = _pixelData[i][j].get_blue() * 0.0722;
				uchar g = _pixelData[i][j].get_green() * 0.7152;
				uchar r = _pixelData[i][j].get_red() * 0.2126;
				uchar grey = b + r + g;
				...
				_pixelData[i][j] = { grey,grey, grey, _pixelData[i][j].alpha };
			}
		}
...
uchar Pixel::get_blue() const
{
	return blue;
}

uchar Pixel::get_red() const
{
	return red;
}

uchar Pixel::get_green() const
{
	return green;
}
